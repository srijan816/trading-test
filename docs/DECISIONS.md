...
## 2026-03-21 — Phase 3E: metric-specific sigma feedback and faster calibration loop
Changed: `src/arena/calibration/crps_tracker.py`, `src/arena/calibration/resolution_hook.py`, `src/arena/data_sources/weather_ensemble.py`, `src/arena/strategies/algo_forecast.py`, `src/arena/intelligence/info_packet.py`, `src/arena/dashboard/queries.py`, `src/tests/test_calibration_feedback.py`
What: Split weather sigma feedback by metric so resolved `highest temperature` and `lowest temperature` markets no longer share a single city-wide sigma recommendation. Settlement now records `metric` in CRPS/Brier history, extracts high/low means and sigmas from strategy evidence, and writes `ensemble_sigma_high` or `ensemble_sigma_low` adjustments per city. `weather_ensemble.py` now loads the latest metric-specific sigma on the next forecast cycle, falling back to the legacy `ensemble_sigma` row when needed. The adjustment curve was also made continuous and more aggressive for badly under-dispersed forecasts, so extreme CRPS ratios widen sigma faster instead of waiting through many settlements. Strategy and info-packet paths now consume the correct metric-specific sigma, and the dashboard calibration query accepts the new parameter names.
Status: `py_compile` passed for all touched modules. Direct regression harnesses passed for the new sigma-adjustment curve, metric-specific DB loading, and evidence parsing.

## 2026-03-21 — Phase 3F: discovery scout for adjacent markets, new weather cities, and cross-venue watchlists
Changed: `src/arena/intelligence/discovery.py`, `src/arena/intelligence/discovery_scout.py` (new), `src/arena/main.py`, `src/arena/scheduler.py`, `config/arena.toml`, `src/tests/test_discovery_scout.py`
What: Added a scheduled `DiscoveryUniverseScanner` that broadens research coverage without enabling new execution. Every hour it now scans active Polymarket markets and logs watch-only `discovery_alerts` for: 1) adjacent liquid categories (`crypto`, `politics`, `economics`) that look structurally suitable for future research, 2) weather cities that have active markets but no forecast-history support or missing coordinates, and 3) cross-venue weather gaps when the Kalshi adapter is actually available. Two new discovery signal types, `market_expansion` and `cross_venue`, keep these alerts visible in the existing dashboard pipeline instead of inventing a separate table.
Status: `py_compile` passed for the scout, scheduler, and main wiring. Direct regression harnesses passed for the ranking heuristic, YES/NO ask extraction, weather-city expansion detection, and a temp-DB end-to-end scout run.

## 2026-03-21 — Phase 3G: circuit breakers, daily P&L guardrails, concentration caps, graceful shutdown, degraded-research fallback
Changed: `src/arena/risk/trading_guardrails.py` (new), `src/arena/risk/risk_manager.py`, `src/arena/intelligence/info_packet.py`, `src/arena/main.py`, `.env.example`, `src/tests/test_trading_guardrails.py`
What: Added a real execution circuit breaker that pauses a strategy for 5 minutes after 5 consecutive failed executions and blocks fresh trades while the pause is active. Replaced the old “daily loss” proxy based on gross spend with a better daily P&L check using today’s realized position P&L plus current unrealized exposure, and added a weather-specific concentration cap for any single city/date bucket. The scheduler now installs `SIGTERM`/`SIGINT` handlers, records a graceful-shutdown event, and cancels open paper limit orders before exit. On the research side, Nexus/Perplexia failures now emit `research_provider_degraded` events and fall back to stale cached research whenever available instead of failing hard.
Status: `py_compile` passed for all touched modules. Direct regression harnesses passed for pause triggering, active-pause lookup, daily P&L computation, and weather city/date concentration rejection.

## 2026-03-21 — Phase 3H: cache hot public market-data paths
Changed: `src/arena/exchanges/polymarket_limit.py`, `src/arena/main.py`, `src/tests/test_public_reader_cache.py`
What: Added a short-lived cache to the public Polymarket reader so repeated `/book`, `/midpoint`, `/price`, and `/tick-size` calls do not hammer the unauthenticated CLOB endpoints during the same scheduler cycle. Orderbooks, midpoint, and side-price reads now reuse responses for 5 seconds; tick size caches for 5 minutes. `execute_decision()` also keeps a per-decision in-memory orderbook cache, avoiding duplicate fetches when multiple actions reference the same venue/market/outcome within one execution pass.
Status: `py_compile` passed for the reader, main execution path, and cache regression test. Direct regression harness confirmed cache hits for repeated orderbook/tick-size reads and a refetch once the cached orderbook entry is expired.

## 2026-03-21 — Phase 3I: maker-flow integration test and orderbook replay harness
Changed: `src/arena/backtest/__init__.py` (new), `src/arena/backtest/orderbook_replay.py` (new), `src/arena/engine/settlement.py`, `src/tests/test_paper_limit_integration.py`, `src/tests/test_orderbook_replay.py`
What: Added a true paper-maker integration test that places a resting limit order, advances the simulated orderbook until it fills, verifies position/execution persistence, and then settles the market to confirm realized P&L bookkeeping. Also added a lightweight orderbook replay harness for historical maker-order evaluation, with a regression test that replays snapshots and scores realized P&L against a settlement outcome. While running the new integration coverage, a real bug surfaced in `SettlementEngine`: the synchronous resolution-hook fallback created an un-awaited coroutine. That path now uses `asyncio.run(...)`, so settlement-triggered calibration feedback executes correctly outside a running loop.
Status: `py_compile` passed for the backtest harness, settlement engine, and new tests. Direct regression harnesses passed for the maker lifecycle test and the replay harness.
